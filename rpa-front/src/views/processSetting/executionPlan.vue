<template>
  <div class="spl-container">
    <breadcrumb :data="pathData">
      <!-- <el-button type="primary" slot="breadcrumb-btn" size="mini" @click="save">确定保存</el-button> -->
    </breadcrumb>
    ...
    <div class="table-box">
      <div>
        <div style="display: flex; justify-content: space-between">
          ...
          <div
            style="color: #3e82ff; cursor: pointer"
            @click="currencyPlan"
            attr="popover"
          >
            设置
          ...
          >
            ...
            >
              ...
              <el-table-column prop="schedule" label="执行计划">
                <template scope="scope">
                  <span v-if="scope.row.robotAppSchedule.taskType===2" class="ic-cust" title="自定义计划"><i class="el-icon-star-on font-12" style="color: #ffffff"></i></span>
              ...
              >
                <template scope="scope">
                  <span>{{ scope.row.robotAppSchedule.editTime ? moment(scope.row.robotAppSchedule.editTime).format('YYYY-MM-DD HH:mm:ss') : '' }}</span>
                </template>
              </el-table-column>
    ...
    > -->
    <el-dialog
      :title="planType == 'currencyProcess'
              ? '设置通用计划'
              : planType == 'addProcess'
              ? '添加计划'
              : '编辑计划'"
        ...
        <div style="margin-top: 20px">
          ...
          >
            ...
            <el-form-item
              label="计划类型："
              prop="planType"
              v-if="planType == 'addProcess' || planType == 'editProcess'"
            >
              ...
              <div v-if="formData.timeType != '3'">
                ...
                <div v-else>
                  <el-form-item label="添加时间点：" prop="timeArr">
                    ...
                    >
                      <div
                        v-for="(item, index) in formData.timeArr"
                        :key="item"
                        class="tag-box"
                      >
                        <div
                        ...
                        <i
                          class="el-icon-remove remove-tag"
                      ...
                      >
                        <div style="display:flex;align-items:center">
                          ...
                          >
                            <el-button
                              size="mini"
                              @click="cancalSelectTimeArr"
                              >取消</el-button
                            >
                            <el-button
                              type="primary"
                              size="mini"
                              @click="confirmSelectTimeArr"
                              >确定</el-button
                            >
                          </div>
                        </div>
            ...
            <!-- <div style="display: flex; justify-content: center">
              <el-button size="mini" style="margin: 0 10px" @click="cancel"
                >取消</el-button
              >
              <el-button
                type="primary"
...
export default {
  ...
  created() {
    ...
    if(this.$route.query.name){
      this.pathData.push({ name: 'RPA执行计划(' + this.$route.query.name + ')'})
    }else{
      this.pathData.push({ name: 'RPA执行计划'})
    }
    ...
    //   if (
    //     e.target.getAttribute('attr') != 'popover' &&
    //     e.target.getAttribute('class') != 'tooltip'
    //   ) {
    //     this.visible2 = false
    //   }
    // }
  ...
  methods: {
    ...
    async currencyPlan(event) {
      ...
      // let dom = this.$refs['processPopover']
      // dom.referenceElm = event.target
      // dom.createPopper() // 这个是必须的，不然是不会进行重新计算的
      // dom.doShow()
      this.visible2 = true
    },
    //添加流程计划
    addPlan() {
      ...
      // dom.referenceElm = window.event.target
      // dom.createPopper() // 这个是必须的，不然是不会进行重新计算的
      // dom.doShow()
    },
    openAddTag() {
    ...
    getPlanInfo() {
      this.showLoading()
      this.$http({
        url: '/robot/flow/flowScheduleList',
        method: 'post',
        params: {
    ...
    setProcess() {
      ...
      this.showLoading()
      this.$http({
        url: '/robot/flow/addFlowSchedule?appCode=' + this.appCode,
        method: 'post',
        data: obj,
      })
        .then((res) => {
    ...
    editProcess(){
      ...
      this.showLoading()
      this.$http({
        url: `/robot/flow/editFlowSchedule/${this.appCode}`,
        method: 'post',
        data: obj,
      }).then((res) => {
          console.log(res)
    ...
    edit(row, index) {
      ...
      // let dom = this.$refs['processPopover']
      // var win = window.event.target
      this.$http({
        url: '/robot/flow/editEcho',
        method: 'post',
        params: {
          flowCode: row.robotAppSchedule.flowCode,
        },
        ...
        this.closeLoading()
        if(data.data && data.code == 200){
          this.formData.planType = data.data.robotAppSchedule.taskType + ''
          this.formData.timeType = data.data.robotAppSchedule.execStyle + ''
          this.formData.id = data.data.robotAppSchedule.id
          var arr1 = data.data.robotAppSchedule.execCycle.split('-')
          this.formData.date1 = Number(arr1[0])
          this.formData.date2 = Number(arr1[1])
          if(this.formData.timeType == 1){
            var arr2 = data.data.robotAppSchedule.execAreaTime.split('-')
            this.formData.time1 = parseInt(arr2[0].split(':')[0],10)
            this.formData.time2 = parseInt(arr2[1].split(':')[0],10)
          }else if(this.formData.timeType == 2){
            var arr2 = data.data.robotAppSchedule.execAreaTime.split(',')
          ...
          this.formData.id = data.data.robotAppSchedule.id
          this.formData.flowCode = data.data.robotAppSchedule.flowCode
          this.formData.title = data.data.flowName
          // console.log(this.formData)
          this.$nextTick(()=>{
            // dom.referenceElm = win
            // dom.createPopper() // 这个是必须的，不然是不会进行重新计算的
            // dom.doShow()
            this.visible2 = true
          })
