    ...
    <el-dialog title="登录验证" :visible.sync="show" class="cust-dialog" width="600px" :close-on-click-modal="false">
      <div style="padding: 0 10px 20px 10px;" v-if="row">
        ...
        <div class="pl-10 pt-10">
          <p class="row">参保城市：<span class="sel">{{row.appName}}</span></p>
          <p class="row">申报账户：<span class="sel">{{row.accountAndOrgName}}</span></p>
          <p class="row">申报系统：<span class="sel">{{row.declareSystemName}}</span></p>
          <p class="row">执行事项：<span class="sel">登录</span></p>
...
export default {
  ...
  methods: {
    ...
    doStartLogin () {
      let that = this
      this.$http({
        url: '/robot/sessionKeep/startLogin',
        method: 'post',
        params: {
          id: this.row.id
        },
