<template>
  <div class="spl-container">
    ...
    <div class="pl-20 pr-20" v-if="formData.type == 'edit'">
      <el-row style="padding-top: 10px">
        ...
        <el-col :span="6">
          <div class="search-row display-flex" style="align-items: center">
            ...
            <span class="label">{{businessType(formData.businessType)}}</span>
          </div>
        </el-col>
        <el-col :span="6">
          <div class="search-row display-flex" style="align-items: center">
        ...
        <el-col :span="6">
          <div class="search-row display-flex" style="align-items: center">
            <span class="ml-20 label">申报单位：</span>
            <span class="label">
              {{
              formData.orgName
              }}
      ...
      <el-row style="padding-top: 10px">
        <el-col :span="6">
          <div class="search-row display-flex ml-20">
            <el-checkbox
              v-model="formData.customMadeField"
              :true-label="1"
              :false-label="0"
              @change="changeCustomMadeField"
            >只看定制字段</el-checkbox>
          </div>
        </el-col>
      </el-row>
    </div>
    <div class="pl-20 pr-20" v-else>
      <el-row style="padding-top: 20px">
        <el-col :span="8">
          <div class="search-row display-flex">
            <span
              class="ml-20 label required"
              style="width: 90px;flex-shrink: 0;text-align: right;"
            >参保城市：</span>
            <addrSelector
        ...
        <el-col :span="6">
          <div class="search-row display-flex" style="align-items: center;">
            ...
            <el-radio-group v-model="searchData.businessType" @change="changeSearchInfo">
        ...
        <el-col :span="9">
          <div class="search-row display-flex" style="align-items: center;">
            ...
            <el-radio-group v-model="searchData.changeType" @change="changeSearchInfo">
              <el-radio :label="item.id" v-for="(item,index) in typeArr" :key="index">{{item.name}}</el-radio>
            </el-radio-group>
          </div>
        </el-col>
        <el-col :span="8">
          <div class="search-row display-flex" style="align-items: center;">
            <span
              class="ml-20 label required"
              style="flex-shrink: 0;width: 90px;text-align: right;"
            >申报单位：</span>
            <el-select v-model="searchData.declareBaseId" clearable filterable style="width:260px;">
              <el-option
        ...
        <el-col :span="8">
          <div class="search-row display-flex" style="align-items: center;">
            ...
            <el-checkbox
              v-model="searchData.customMadeField"
              :true-label="1"
              :false-label="0"
              @change="changeCustomMadeField"
            >只看定制字段</el-checkbox>
    ...
    <div class="table-box">
      ...
      <el-form ref="tableForm" :model="table" label-width="0px" :rules="rules">
        ...
        >
                ...
                >
                  <div>
                    <el-select
                      ...
                      placeholder="请选择"
                      size="mini"
                      :disabled="scope.row.customMadeField!==1"
                      @change="dataSourceTableCodeChange($event, scope.$index)"
                    >
                      <el-option
                        v-for="item in scope.row.dataSourceOption"
                        :key="item.id"
                        :label="item.name"
                        :value="item.code"
                        :disabled="scope.row.customMadeField!==1"
                ...
                >
                  <div style="margin-left: 10px">
                    <el-select
                      ...
                      size="mini"
                      :disabled="scope.row.customMadeField!==1"
                      @change="dataSourceItemCodeChange($event, scope.row)"
                    >
                      <el-option
          ...
          <el-table-column prop="address" label="字段属性" width="280">
            <template scope="scope">
              <div style="display: flex; align-items: center">
                ...
                >
                  <div>
                    <el-select
                      ...
                      :disabled="scope.row.disabledSelect || scope.row.customMadeField!==1"
                    >
                      <el-option
                        v-for="item in filterValueTypeOption1(scope.row.dataSourceItemCode,scope.$index)"
                        :key="item.value"
                        :label="item.label"
                        :value="item.value"
                      ></el-option>
                ...
                >
                  <div style="margin-left: 10px">
                    <el-select v-model="scope.row.exportFormat" placeholder="请选择" size="mini" :disabled="scope.row.customMadeField!==1">
                      <el-option
          ...
          <!-- <el-table-column prop="name" label="条件" min-width="150">
            <template scope="scope">
              <span
          ...
          <el-table-column prop="comment" label="是否定制字段" width="120">
            <template scope="scope">
              <div style="display: flex">
                <el-checkbox v-model="scope.row.customMadeField" :true-label="1" :false-label="0"></el-checkbox>
              </div>
            </template>
          </el-table-column>
          <el-table-column prop="address" label="操作" width="0">
            <template scope="scope">
              <div style="display: flex" v-if="scope.row.customMadeField == 1">
                <i class="el-icon-edit" style="color:#3E82FF" @click="showDrawer(scope.$index)"></i>
                <!-- <div
                  class="control-btn"
                  style="color: red"
          ...
          <div ref="outPutFormTop">
            <div style="display: flex; justify-content: space-between">
              <el-form-item prop="declareColumnName" label="字段名称">
                <el-input
                  v-model.trim="outputForm.declareColumnName"
                  placeholder="请输入字段名称"
                  :disabled="outputForm.disabled"
                ></el-input>
              </el-form-item>
              <el-form-item prop="comment" label="注解" style="width: 50%">
                <el-input
                  v-model="outputForm.comment"
                  placeholder="请输入注解"
                  :disabled="outputForm.disabled"
                ></el-input>
              </el-form-item>
...
export default {
  ...
  computed: {
    ...
    tableHeight1() {
      return this.$global.bodyHeight - 320 + this.outputTable1
    },
    tableHeight2() {
    ...
    filterValueTypeOption1() {
      return function (code, index) {
        if (this.table.tableData[index].dataSourceTableCode == this.table.tableDataCache[index].dataSourceTableCode) {
          return this.valueTypeOption1.filter((item) => item.value == this.table.tableDataCache[index].inputType)
        }
        if (code == '20000002') {
  ...
  async created() {
    ...
    this.pathData.push({ path: '/offerFile/customizedList', name: '定制报盘' })
    for (const key in this.$route.query) {
      this.formData[key] = this.checkRouteQuery(key)
    }
    if (this.formData.type == 'edit') {
      await this.requestInfo().catch((err) => {
        console.log(err)
      })
      this.requestInfo2(this.table.declareBaseUuid)
      this.pathData.push({ name: '编辑定制报盘' })
    } else {
      this.getAddr()
      this.getOrgList()
      this.pathData.push({ name: '新增定制报盘' })
    }
  },
  methods: {
    //只看定制字段
    changeCustomMadeField(val) {
      if (val == 1) {
        this.temporary = this.deepCopy(this.table.tableData)
        this.table.tableData = this.table.tableData.filter((item) => item.customMadeField == 1)
      } else {
        this.table.tableData = this.temporary
      }
      console.log(this.table.tableData, this.formData)
    },
    ...
    changeInputType(val, index) {
      ...
      if (val == 2 && this.table.tableData[index].dataSourceTableCode != '60000') {
        this.dataSourceItemCodeChange(this.table.tableData[index].dataSourceItemCode, this.table.tableData[index])
    ...
    dataSourceTableCodeChange(e, index) {
      ...
      var data = this.table.tableData[index]
      var cache = this.table.tableDataCache[index]
      //如果选其他 按原来的逻辑
      if (data.dataSourceTableCode == '50000') {
        this.table.tableData[index].dataSourceOption2 = this.dataSourceOptionItem[data.dataSourceTableCode]
        this.table.tableData[index].valueTypeOption2 = this.valueTypeOption2Item[data.inputType] ? this.valueTypeOption2Item[data.inputType] : []
      } else {
        //如果没选其他，则还原之前的配置，且不能修改 所以对下拉选项筛选成只有一个了
        console.log(this.dataSourceOptionItem[data.dataSourceTableCode],data)
        this.table.tableData[index].dataSourceOption2 = this.dataSourceOptionItem[data.dataSourceTableCode]
          ? this.dataSourceOptionItem[data.dataSourceTableCode].filter((item) => cache.dataSourceItemCode == item.code )
          : []
          console.log(this.table.tableData[index].dataSourceOption2,this.valueTypeOption2Item[cache.inputType],cache)
        if(this.valueTypeOption2Item[cache.inputType]){
          this.table.tableData[index].valueTypeOption2 = this.valueTypeOption2Item[cache.inputType].filter((item) => {
            return cache.exportFormat == item.dictCode
          })
        }
        this.table.tableData[index].exportFormat = cache.exportFormat
        this.table.tableData[index].inputType = cache.inputType
      }
      ...
      this.$nextTick(() => {
        if (this.table.tableData[index].dataSourceOption2.length > 0) {
          ...
          if (data.dataSourceTableCode == cache.dataSourceTableCode) {
            this.table.tableData[index].columnScopes = this.deepCopy(cache.columnScopes)
            this.table.tableData[index].inputType = cache.inputType
          } else {
          ...
          //   this.table.tableData[index].disabledSelect = false
          // }
          if (this.table.tableData[index].dataSourceOption2[0].type == 0) {
            this.table.tableData[index].inputType = 1
            this.table.tableData[index].disabledSelect = false
          } else {
            this.table.tableData[index].disabledSelect = false
    ...
    dataSourceItemCodeChange(val, row) {
      // console.log(val,row)
    ...
    async requestInfo() {
      this.showLoading()
      let url = 'policy/customMade/offerSettings/findByUuid/' + this.$route.query.uuid
      if (this.formData.type != 'edit') {
        url = `policy/customMade/offerSettings/findCustomMadeSetting/${this.searchData.addrId}/${this.searchData.businessType}/${this.searchData.changeType}`
      }
      return this.$http({
        url: url,
        method: 'post',
        ...
        .then(async ({ data }) => {
          if (!data.data.declareBaseUuid) {
            this.formData = this.$message.warning('该地区暂无报盘数据')
            this.formData = this.$options.data().formData
            this.table = this.$options.data().table
            this.closeLoading()
            return
          }
          this.table.declareBaseUuid = data.data.declareBaseUuid
          data.data.showType = data.data.showType == null ? 0 : data.data.showType
          if (this.formData.type == 'edit') {
            this.formData = Object.assign(data.data, this.formData)
          } else {
          ...
          this.importData.systemList.length <= 0 && (await this.getIntroduction(data.data.addrId))
          if(this.dataSourceOption.length <= 0 || this.cacheBusinessType != this.formData.businessType){
            await this.getFindAllDataSource()
          }
          !this.valueTypeOption2Item[3] && (await this.getDictByKey('10001', '3'))
          !this.valueTypeOption2Item[4] && (await this.getDictByKey('10006', '4'))
          this.cacheBusinessType = this.formData.businessType
          try {
            data.data.columnSettings.forEach((item, index) => {
              item.dataSourceOption2 = this.dataSourceOptionItem[item.dataSourceTableCode]
                ? this.dataSourceOptionItem[item.dataSourceTableCode].filter((item1) => {
                    return item.dataSourceItemCode == item1.code
                  })
                : []

              item.valueTypeOption2 = this.valueTypeOption2Item[item.inputType]
                ? this.valueTypeOption2Item[item.inputType].filter((item1) => item1.dictCode == item.exportFormat)
                : []

              item.dataSourceOption = this.dataSourceOption.filter((item1) => {
                return item.dataSourceTableCode == item1.code || item1.code == '50000'
              })
          ...
          // console.log(data.data.columnSettings)
          this.table.tableData = data.data.columnSettings
          this.table.tableDataCache = this.deepCopy(data.data.columnSettings)
          this.closeLoading()
        })
        .catch(() => {
          this.closeLoading()
    ...
    async requestInfo2(uuid) {
      this.showLoading()
      return this.$http({
        url: 'policy/offerSettings/findByUuid/' + uuid,
        method: 'post',
      })
        .then(async ({ data }) => {
          this.table.tableDataCache = data.data.columnSettings
          this.table.tableData.forEach((item,index)=>{
            this.table.tableDataCache.forEach(item1=>{
              if(item.declareColumnUuid == item1.uuid){
                item.prevDataSourceTableCode = item1.dataSourceTableCode
                item.prevDataSourceItemCode = item1.dataSourceItemCode
                item.prevInputType = item1.inputType
                item.prevExportFormat = item1.exportFormat
                this.table.tableData[index].dataSourceOption= this.dataSourceOption.filter((item2) => {
                  return item.dataSourceTableCode == item2.code || item2.code == '50000' || item.prevDataSourceTableCode == item2.code
                })
                //如果和报盘信息，不一样，则认为是其他，其他可展示全部 信息
                if(item.dataSourceTableCode != item.prevDataSourceTableCode){
                  this.table.tableData[index].dataSourceOption2 = this.dataSourceOptionItem[item.dataSourceTableCode]
                  this.table.tableData[index].valueTypeOption2 = this.valueTypeOption2Item[item.inputType]
                  item.columnScopes.forEach((item1) => {
                      item1.readonly = false
                  })
                }else{
                  //如果是，则展示唯一信息
                  this.table.tableData[index].dataSourceOption2 = this.dataSourceOptionItem[item.dataSourceTableCode] ? this.dataSourceOptionItem[item.dataSourceTableCode].filter(item1=>{
                    return item.dataSourceItemCode == item1.code
                  }) : []
                  item.columnScopes.forEach((item1) => {
                    item1.readonly = true
          ...
          this.table.tableData = Object.assign([],this.table.tableData)
          console.log(this.table.tableData)
          this.closeLoading()
        })
        .catch((err) => {
    ...
    //确定保存
    save() {
      this.$refs.tableForm.validate(async (vaild) => {
        if (vaild) {
          if (this.formData.type != 'edit') {
            this.formData.declareBaseId = this.searchData.declareBaseId
          }

          if (!this.searchData.addrId && this.formData.type != 'edit') {
            this.$message.warning('请选择参保城市')
            return
          }
          if (!this.searchData.declareBaseId && this.formData.type != 'edit') {
            this.$message.warning('请选择申报单位')
            return
          }
          if (this.formData.type != 'edit') {
            var mode = await this.validateCustomMade()
            if (mode == 1) {
              this.$message.warning('此申报单位已经设置该地区的定制报盘')
              return
            }
          }
          var check = true
          var err = []
          this.formData.columnSettings = this.table.tableData.filter((item) => item.customMadeField == 1)
          var dictCode = this.formData.columnSettings.map((item) => {
            return item.dataSourceItemCode
          })
          if (this.formData.columnSettings.length <= 0) {
            this.$message.error('请至少设置一条定制字段数据')
            return
          }
          this.formData.columnSettings.forEach((item, index) => {
            ...
            //   }
            // })
            item.columnScopes = item.columnScopes ? item.columnScopes.filter(item=>item.scopeValue) : []
    ...
    async getIntroduction(addrId) {
      this.$http({
        url: 'policy/offerSettings/getIntroduction/' + this.formData.addrId + '/' + this.formData.businessType,
        method: 'post',
      }).then(({ data }) => {
    ...
    async getFindDataSourceItem(tableCode) {
      ...
      await this.$http({
        ...
        method: 'post',
        params: {
          bussinssType: this.formData.businessType,
    ...
    saveAll() {
      ...
      this.showLoading()
      this.$http({
        url: 'policy/customMade/offerSettings/addOrUpdate',
        method: 'post',
        data: this.formData,
      })
        .then((res) => {
    ...
    autoSort() {
      // var arr = ['单位社保号','单位公积金号','姓名','证件号码','参保时间','参保险种','公积金单位比例','公积金个人比例','补充公积金单位比例','补充公积个人比例','缴纳基数']
      var arr = ['30000003', '30000004', '10000001', '10000003', '20000002', '40000011', '40000007', '40000008', '40000009', '40000010', '20000005']
      var tmpArr = []
      this.table.tableData.forEach((item, index) => {
    ...
    checkRouteQuery(key) {
      return this.$route.query[key] ? this.$route.query[key] : ''
    },
    //获取申报单位
    getOrgList() {
      this.$http({
        ...
        .then((res) => {
          ...
          if (!this.changeBusinessType()) {
            this.searchData.declareBaseId = ''
          }
        })
        .catch((item) => {})
    },
    ...
    changeBusinessType() {
      var check = false
      this.customMadeList.forEach((item) => {
        if (item.id == this.searchData.declareBaseId) {
          check = true
        }
      })
      return check
    ...
    validateCustomMade() {
      return this.$http({
        url: 'policy/customMade/offerSettings/validateCustomMade',
        method: 'post',
        params: {
          addrId: this.searchData.addrId,
          businessType: this.searchData.businessType,
          changeType: this.searchData.changeType,
          declareBaseId: this.searchData.declareBaseId,
        },
      })
        .then((res) => {
