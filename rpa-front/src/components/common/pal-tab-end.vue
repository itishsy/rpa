...
<template>
  <div :class="'pal-tabs display-flex pal-tabs' + type">
    <span class="span-tabs" :class="{'marginEnd':index === tabs.length -1}" v-for="(item, index) in tabs" :key="index" @click="handleClickTabs(index, item)">
      <div v-if="enableTips">
    ...
    <div class="flex1 pt-10 text-right pr-20" v-if="!custom">
      <slot name="btns"></slot>
    </div>
  </div>
</template>
