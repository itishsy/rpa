...
class AiUiAutomation(object):
    ...
    @staticmethod
    def openApplication(app, Name=None, ClassName=None, searchDepth=1, timeout=15):
        """
        @summary: 打开应用
    ...
    @staticmethod
    def StatusBarControl(parent: ui.WindowControl, Name=None, ClassName=None, timeout=3):
        """
        @summary: 获取按钮控件对象
        @param parent: 按钮所在窗口对象
        @param Name: 按钮名称
        ...
        """
        if Name is not None:
            statusBar = parent.StatusBarControl(Name=Name)
        if ClassName is not None:
            statusBar = parent.StatusBarControl(ClassName=ClassName)
        if statusBar.Exists(timeout):
            return statusBar
        return None

    @staticmethod
    def TextControl(parent: ui.WindowControl, Name=None, timeout=3):
        """
        @summary: 获取按钮控件对象
        @param parent: 按钮所在窗口对象
        @param Name: 按钮名称
        ...
        @return:
        """
        text = parent.TextControl(Name=Name)
        if text.Exists(timeout):
            return text
        return None
...
if __name__ == '__main__':
    AiUiAutomation.setLogPath(r'E:\logs\rpa')
    app = r'C:\Program Files (x86)\SHGJJ\gjj.exe'
    sta, window = AiUiAutomation.openApplication(app, ClassName="TfLoginFirst")
    if sta:
        firstChoiceWindow = AiUiAutomation.WindowControl(ClassName="TfLoginFirst")
        button = AiUiAutomation.ButtonControl(firstChoiceWindow, Name="    确定", timeout=10)
        AiUiAutomation.clickButton(button, waitTime=3)

        PINWindow = AiUiAutomation.WindowControl(Name="PIN码确认", ClassName="TForm")
        password = AiUiAutomation.EditControl(PINWindow, ClassName="TEdit")
        password.SendKeys('143a5f1b', waitTime=5)

        submitBtn = AiUiAutomation.ButtonControl(PINWindow, Name="确定")
        AiUiAutomation.clickButton(submitBtn, waitTime=20)

        mainWindow = AiUiAutomation.WindowControl(ClassName="Gjj")
        statusBar = AiUiAutomation.StatusBarControl(mainWindow, ClassName="TStatusBar")
        print(statusBar.GetWindowText())
        print(statusBar.GetLastChildControl().GetWindowText())
        textVal = AiUiAutomation.EditControl(statusBar, ClassName=None)
