...
public class FlowExecutor {
    ...
    public void start(List<RobotFlowStep> steps) {
        ...
        //结束步骤
        RobotFlowStep end = steps.stream().filter(s -> RobotConstant.END_ACTION_CODE.equals(s.getActionCode())).findFirst().get();
        try {
            //开始步骤
            RobotFlowStep step = steps.stream().filter(s -> RobotConstant.START_ACTION_CODE.equals(s.getActionCode())).findFirst().get();
            do {
                RobotExecutionDetail detail = null;

                try {
                    if (StringUtils.isBlank(step.getSkipTo())) {
                        throw new RobotRuntimeException("步骤：" + step.getStepName() + " 下一步指令获取异常");
                    }

                    if (step.getStatus() == 0) {
                        if ("match".equals(step.getActionCode())) {
                            throw new RobotRuntimeException("步骤：" + step.getStepName() + " 为判断指令，不能禁用");
                        } else {
                            throw new RuntimeSkipTo(step.getSkipTo());
                        }
                    }
                    ...
                    step = Convert.getNextStep(steps, step.getSkipTo());
                } catch (RuntimeSkipTo rst) {
                    if (detail != null) {
                        detail.setStatus(1);
                    }
                    String skipTo = rst.getStepName();
                    if (RobotConstant.NEXT_STEP.equals(skipTo)) {
                        skipTo = step.getSkipTo();
                    }
                    step = Convert.getNextStep(steps, skipTo);
                } catch (Exception e) {
                    if (detail != null) {
                        detail.setStatus(0);
                        ...
                        if (e.getCause() != null) {
                            ...
                            traceElements = e.getCause().getStackTrace();
                        } else {
                            errorInfo.append(e.getMessage());
                            traceElements = e.getStackTrace();
                        }
                        //控制字符串过长，只读取10行，如需更多，需要获取日志文件
                        int len = traceElements.length > 10 ? 10 : traceElements.length;
                        for (int i = 0; i < len; i++) {
                            errorInfo.append(traceElements[i].toString());
                        }
                        detail.setError(errorInfo.toString());
                    }
                    throw e;
                } finally {
                    if (detail != null) {
            ...
            } while (step != null && !step.getStepCode().equals(end.getStepCode()));
        } catch (Exception e) {
            //如果是子流程，异常往外抛。
            if (isSubFlow) {
    ...
    private void runAction(RobotFlowStep step) {
        ...
        // 执行前等待
        this.randomWait(step.getWaitBefore());

        if (StringUtils.isNotBlank(step.getSkipCondition()) && ELParser.parse(step.getSkipCondition(), ctx.getVariables(), Boolean.class)) {
            throw new RuntimeSkipTo(step.getSkipTo());
        }
        ...
        //执行后等待.
        this.randomWait(step.getWaitAfter());
    }
