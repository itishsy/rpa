...
public class DebugExecutor {
    ...
    public void start(List<RobotFlowStep> steps) {
        ...
        try {
            ...
            for (int s = 0; s < size; s++) {
                ...
                if (stop) {
                    throw new RobotRuntimeException("手动停止");
                }
                ...
                //最后子流程
                if (FINAL_ACTION_CODE.equals(step.getActionCode())) {
                    continue;
                }
                ...
                try {
                    if (ctx.contains(RobotConstant.EXECUTION_CODE_KEY)) {
                        ...
                        detail.setFlowCode(step.getFlowCode());
                        detail.setStepCode(step.getStepCode());
                        detail.setStepName(step.getStepName());
                        detail.setStartTime(new Date());
                    }
                    DebugContext.stepVariables.put(step.getStepCode(), ctx.getVariables());
                    runAction(step);
                    if (ctx.getVariable("offerIteratorCode") != null) {
                        if (step.getLevel() == 1 && (s + 1 >= size || steps.get(s + 1).getLevel() == 0)) {
                            System.out.println("遍历器元素最底部");
                            int offerIteratorSize = Integer.parseInt(ctx.getVariable("offerIteratorSize").toString());
                            int offerIteratorIndex = Integer.parseInt(ctx.getVariable("offerIteratorIndex").toString());
                            if (offerIteratorIndex + 1 < offerIteratorSize) {
                                ctx.setVariable("offerIteratorIndex", offerIteratorIndex + 1);
                                System.out.println("索引：" + offerIteratorIndex);
                                String stepCode = ctx.getVariable("offerIteratorCode").toString();
                                int index = -1;
                                for (int i = 0; i < steps.size(); i++) {
                                    if (stepCode.equals(steps.get(i).getStepCode())) {
                                        index = i;
                                        break;
                                    }
                                }
                                if (index == -1) {
                                    throw new BusinessException("找不到对应的 step:{}", stepCode);
                                }
                                String nextStepName = "";
                                for (int i = index + 1; i <= s; i++) {
                                    if (steps.get(i).getStatus() == 1) {
                                        nextStepName = steps.get(i).getStepName();
                                        break;
                                    }
                                }
                                throw new RuntimeSkipTo(nextStepName);
                            } else {
                                ctx.remove("offerIteratorSize");
                                ctx.remove("offerIteratorIndex");
                                ctx.remove("offerIteratorCode");
                    ...
                    String stepCode = ers.getStepCode();
                    skipTo = null;
                    // 寻找一个外层的指令
                    String nextStepName = "";
    ...
    private void runAction(RobotFlowStep step) {
        ...
        //断点调试
        boolean debugFlag = this.debugFlag(step.getStepCode());

        // 执行前等待
        randomWait(debugFlag, step, step.getWaitBefore());
        ...
        //执行后等待.
        randomWait(false, step, step.getWaitAfter());
    }
     ...
     */
    private void randomWait(boolean debugFlag, RobotFlowStep step, Integer seconds) {
        ...
        try {
            if (debugFlag || (StringUtils.isNotBlank(DebugContext.command) && "suspend".equals(DebugContext.command))) {
                synchronized (DebugContext.lock) {
                    DebugContext.lock.acquire();
                    log.info("执行步骤前：" + step.getStepName() + " 断点暂停或手动暂停.");
                }
            } else {
    ...
    private boolean debugFlag(String stepCode) {
        if (!syncInService.isDevEnv("")) {
            return false;
        }
        List<String> debugStepCodes = ctx.get(RobotConstant.DEBUG_STEP_CODES);
        if (CollectionUtils.isNotEmpty(debugStepCodes)) {
            if (debugStepCodes.contains(stepCode)) {
                return true;
            }
        }
        return false;
    ...
    private boolean stop() {
        ...
        if (StringUtils.isNotBlank(DebugContext.command) && "stop".equals(DebugContext.command)) {
            DebugContext.command = null;
            return true;
        }
        return false;
    }
